	...
	public function test__construct()
	{
		$this
			...
			->then
				...
				->object($globIteratorResolver = $runner->getGlobIteratorResolver())->isInstanceOf('mageekguy\atoum\dependencies\resolver')
				->object($globIteratorResolver(array('pattern' => $pattern = uniqid())))->isEqualTo(new \globIterator($pattern))
				->object($reflectionClassResolver = $runner->getReflectionClassResolver())->isInstanceOf('mageekguy\atoum\dependencies\resolver')
				->object($reflectionClassResolver(array('class' => $this)))->isEqualTo(new \reflectionClass($this))
				->object($testResolver = $runner->getTestResolver())->isInstanceOf('mageekguy\atoum\dependencies\resolver')
				->object($test = $testResolver(array('test' => __CLASS__)))->isInstanceOf(__CLASS__)
				->object($test->getAdapter())->isIdenticalTo($runner->getAdapter())
				->object($test->getLocale())->isIdenticalTo($runner->getLocale())
				->object($test->getIncluder())->isIdenticalTo($runner->getIncluder())
			...
			->if($runner = new atoum\runner($resolver = new dependencies\resolver()))
			->then
				->object($runner->getScore())->isInstanceOf('mageekguy\atoum\score')
				->object($runner->getAdapter())->isInstanceOf('mageekguy\atoum\adapter')
				->object($runner->getLocale())->isInstanceOf('mageekguy\atoum\locale')
				...
				->variable($runner->getDefaultReportTitle())->isNull()
				->array($runner->getObservers())->isEmpty()
			->if($resolver['runner\score'] = $score = new atoum\runner\score())
			->and($resolver['adapter'] = $adapter = new atoum\adapter())
			->and($resolver['locale'] = $locale = new atoum\locale())
			->and($resolver['includer'] = $includer = new atoum\includer())
			->and($resolver['test\directory\iterator'] = $testDirectoryIterator = new atoum\iterators\recursives\directory())
			->and($resolver['globIterator\resolver'] = $globIteratorResolver = new dependencies\resolver(function() {}))
			->and($resolver['reflection\class\resolver'] = $reflectionClassResolver = new dependencies\resolver(function($resolver) { return new \reflectionClass($resolver['@class']); }))
			->and($runner = new atoum\runner($resolver))
			->then
				...
				->object($runner->getGlobIteratorResolver())->isIdenticalTo($globIteratorResolver)
				->object($runner->getReflectionClassResolver())->isIdenticalTo($reflectionClassResolver)
				->object($test = $testResolver(array('test' => __CLASS__)))->isInstanceOf(__CLASS__)
				->object($test->getAdapter())->isIdenticalTo($adapter)
				->object($test->getLocale())->isIdenticalTo($locale)
				->object($test->getIncluder())->isIdenticalTo($includer)
				->variable($runner->getRunningDuration())->isNull()
				->boolean($runner->codeCoverageIsEnabled())->isTrue()
				->variable($runner->getDefaultReportTitle())->isNull()
				->array($runner->getObservers())->isEmpty()
