...
class light extends atoum\test
{
	public function test__construct()
	{
		$this
			...
			->and($dependencies['mageekguy\atoum\reports\realtime\cli\light']['locale'] = $localeInjector = function() use (& $locale) { return $locale = new atoum\locale(); })
			->and($dependencies['mageekguy\atoum\reports\realtime\cli\light']['adapter'] = $adapterInjector = function() use (& $adapter) { return $adapter = new atoum\adapter(); })
			->and($report = new reports\realtime\cli\light($dependencies))
			->then
				->object($report->getDepedencies())->isIdenticalTo($dependencies[$report])
				->object($dependencies['mageekguy\atoum\reports\realtime\cli\light']['locale'])->isIdenticalTo($localeInjector)
				->object($dependencies['mageekguy\atoum\reports\realtime\cli\light']['adapter'])->isIdenticalTo($adapterInjector)
