...
class directory extends atoum\test
{
	...
	public function test__construct()
	{
		$this
			->if($iterator = new recursives\directory())
				...
				->object($iteratorDepedencies = $iterator->getDepedencies())->isInstanceOf('mageekguy\atoum\dependencies')
			...
			->then
				...
				->boolean($iterator->dotsAreAccepted())->isFalse()
				->array($iterator->getAcceptedExtensions())->isEqualTo(array('php'))
				->object($iteratorDepedencies = $iterator->getDepedencies())->isIdenticalTo($dependencies[$iterator])
				->boolean(isset($iteratorDepedencies['directory\iterator']))->isTrue()
				->boolean(isset($iteratorDepedencies['filters\dot']))->isTrue()
				->boolean(isset($iteratorDepedencies['filters\extension']))->isTrue()
			->mockGenerator->shunt('__construct')
			->if($dependencies = new atoum\dependencies())
			->and($dependencies['mageekguy\atoum\iterators\recursives\directory']['directory\iterator'] = $directoryIteratorInjector =  function() {})
			->and($dependencies['mageekguy\atoum\iterators\recursives\directory']['filters\dot'] = $dotFilterInjector = function() {})
			->and($dependencies['mageekguy\atoum\iterators\recursives\directory']['filters\extension'] = $extensionFilterInjector = function() {})
			->if($iterator = new recursives\directory($path = uniqid(), $dependencies))
			->then
				->string($iterator->getPath())->isEqualTo($path)
				->boolean($iterator->dotsAreAccepted())->isFalse()
				->array($iterator->getAcceptedExtensions())->isEqualTo(array('php'))
	...
	public function testSetDepedencies()
	{
		$this
			...
			->then
				->object($iterator->setDepedencies($dependencies = new atoum\dependencies()))->isIdenticalTo($iterator)
				->object($iteratorDepedencies = $iterator->getDepedencies())->isIdenticalTo($dependencies[$iterator])
				->boolean(isset($iteratorDepedencies['directory\iterator']))->isTrue()
				->boolean(isset($iteratorDepedencies['filters\dot']))->isTrue()
				->boolean(isset($iteratorDepedencies['filters\extension']))->isTrue()
			...
			->and($dependencies['mageekguy\atoum\iterators\recursives\directory']['filters\extension'] = $extensionFilterInjector = function() {})
			->then
				->object($iterator->setDepedencies($dependencies))->isIdenticalTo($iterator)
				->object($iteratorDepedencies = $iterator->getDepedencies())->isIdenticalTo($dependencies['mageekguy\atoum\iterators\recursives\directory'])
				->object($iteratorDepedencies['directory\iterator'])->isIdenticalTo($directoryIteratorInjector)
				->object($iteratorDepedencies['filters\dot'])->isIdenticalTo($dotFilterInjector)
				->object($iteratorDepedencies['filters\extension'])->isIdenticalTo($extensionFilterInjector)
		;
	...
	public function testGetIterator()
	{
		$this
			...
			->if($dependencies = new \mock\mageekguy\atoum\dependencies())
			->and($dependencies['mageekguy\atoum\iterators\recursives\directory']['directory\iterator'] = function($path) use (& $recursiveDirectoryIterator) { return $recursiveDirectoryIterator = new \mock\recursiveDirectoryIterator($path); })
			->and($dependencies['mageekguy\atoum\iterators\recursives\directory']['filters\dot'] = function($iterator, $dependencies) use (& $dotFilter) { return $dotFilter = new atoum\iterators\filters\recursives\dot($iterator, $dependencies); })
			->and($dependencies['mageekguy\atoum\iterators\recursives\directory']['filters\extension'] = function($iterator, $extensions) use (& $extensionFilter) { return $extensionFilter = new atoum\iterators\filters\recursives\extension($iterator, $extensions); })
