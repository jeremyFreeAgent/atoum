...
class builder extends atoum\test
{
	...
	public function test__construct()
	{
		$this->assert
			...
			->and($dependencies['superglobals'] = $superglobals = new atoum\superglobals())
			->and($dependencies['vcs'] = $vcs = new atoum\scripts\builder\vcs\svn())
	...
	public function testGetPhp()
	{
		$this
			...
			->and($superglobals->_SERVER['_'] = $php = uniqid())
			->and($builder = new scripts\builder(uniqid()))
			->and($builder->setSuperglobals($superglobals))
			->then
				->string($builder->getPhpPath())->isEqualTo($php)
			->when(function() use ($superglobals) { unset($superglobals->_SERVER['_']); })
			->and($builder = new scripts\builder(uniqid()))
			->and($builder->setSuperglobals($superglobals))
			->then
			...
			->if($builder->setPhpPath($php = uniqid()))
			->then
				->string($builder->getPhpPath())->isEqualTo($php)
		;
	}
	...
	public function testCreatePhar()
	{
		$this
			...
			->and($builder->getMockController()->resetCalls())
			->and($taggerEngine->getMockController()->resetCalls())
			->and($vcsController->resetCalls()->getNextRevisions = function() use (& $revision) { static $i = 0; return ++$i > 1 ? array() : array($revision = rand(1, PHP_INT_MAX)); })
			->then
