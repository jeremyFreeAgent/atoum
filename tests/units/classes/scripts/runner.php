...
class runner extends atoum\test
{
	...
	public function test__construct()
	{
		$this
			...
			->then
				...
				->object($scriptRunner->getIncluder())->isIdenticalTo($scriptRunner->getIncluder())
			...
			->and($resolver['runner'] = $runner = new atoum\runner())
			->and($resolver['includer'] = $includer = new atoum\includer())
			->and($scriptRunner = new scripts\runner($name = uniqid(), $resolver))
			->then
				->string($scriptRunner->getName())->isEqualTo($name)
	...
	public function testUseConfigFile()
	{
		$this
			->if($resolver = new dependencies\resolver())
			->and($resolver['locale'] = $locale = new \mock\mageekguy\atoum\locale())
			->and($runner = new scripts\runner(uniqid(), $resolver))
			->then
