...
class script extends atoum\test
{
	...
	public function test__construct()
	{
		$this
			...
			->then
				...
				->object($dependencies = $script->getDepedencies())->isInstanceOf('mageekguy\atoum\dependencies')
			...
			->and($dependencies['mock\mageekguy\atoum\script']['locale'] = $localeInjector = function() use (& $locale) { return $locale = new atoum\locale(); })
			->and($dependencies['mock\mageekguy\atoum\script']['adapter'] = $adapterInjector = function() use (& $adapter) { return $adapter = new atoum\adapter(); })
			->and($dependencies['mock\mageekguy\atoum\script']['arguments\parser'] = $argumentsParserInjector = function() use (& $argumentsParser) { return $argumentsParser = new atoum\script\arguments\parser(); })
			->and($dependencies['mock\mageekguy\atoum\script']['writers\output'] = $outputWriterInjector = function() use (& $outputWriter) { return $outputWriter = new atoum\writers\std\out(); })
			->and($dependencies['mock\mageekguy\atoum\script']['writers\error'] = $errorWriterInjector = function() use (& $errorWriter) { return $errorWriter = new atoum\writers\std\err(); })
			->and($script = new mock\script($name = uniqid(), $dependencies))
			->then
				->string($script->getName())->isEqualTo($name)
				...
				->array($script->getHelp())->isEmpty()
				->object($script->getDepedencies())->isIdenticalTo($dependencies['mock\mageekguy\atoum\script'])
				->object($dependencies['mock\mageekguy\atoum\script']['locale'])->isIdenticalTo($localeInjector)
				->object($dependencies['mock\mageekguy\atoum\script']['adapter'])->isIdenticalTo($adapterInjector)
				->object($dependencies['mock\mageekguy\atoum\script']['arguments\parser'])->isIdenticalTo($argumentsParserInjector)
				->object($dependencies['mock\mageekguy\atoum\script']['writers\output'])->isIdenticalTo($outputWriterInjector)
				->object($dependencies['mock\mageekguy\atoum\script']['writers\error'])->isIdenticalTo($errorWriterInjector)
			->if($adapter = new atoum\test\adapter())
			->and($adapter->php_sapi_name = uniqid())
			->and($dependencies['mock\mageekguy\atoum\script']['adapter'] = $adapter)
			->then
				->exception(function() use ($dependencies, & $name) {
						new mock\script($name = uniqid(), $dependencies);
					}
				)
	...
	public function testSetDepedencies()
	{
		$this
			->if($script = new mock\script($name = uniqid()))
			->then
				->object($script->setDepedencies($dependencies = new atoum\dependencies()))->isIdenticalTo($script)
				->object($script->getDepedencies())->isIdenticalTo($dependencies['mock\mageekguy\atoum\script'])
				->boolean(isset($dependencies['mock\mageekguy\atoum\script']['locale']))->isTrue()
				->boolean(isset($dependencies['mock\mageekguy\atoum\script']['adapter']))->isTrue()
				->boolean(isset($dependencies['mock\mageekguy\atoum\script']['arguments\parser']))->isTrue()
				->boolean(isset($dependencies['mock\mageekguy\atoum\script']['writers\output']))->isTrue()
				->boolean(isset($dependencies['mock\mageekguy\atoum\script']['writers\error']))->isTrue()
		;
	}
	...
	public function testRun()
	{
		$this
			...
			->and($dependencies['mock\mageekguy\atoum\script']['arguments\parser'] = $argumentsParser)
			->and($dependencies['mock\mageekguy\atoum\script']['adapter'] = $adapter = new atoum\test\adapter())
			->and($script = new mock\script(uniqid(), $dependencies))
			->then
	...
	public function testPrompt()
	{
		...
		$this
			...
			->and($dependencies = new atoum\dependencies())
			->and($dependencies['mock\mageekguy\atoum\script']['adapter'] = $adapter)
			->and($dependencies['mock\mageekguy\atoum\script']['writers\output'] = $stdOut)
			->and($script = new mock\script(uniqid(), $dependencies))
			->then
